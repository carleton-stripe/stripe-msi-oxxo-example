...
router.post('/webhook', async (req, res) => {
  ...
  console.log('EVENT TYPE: ' + JSON.stringify(eventType));
...
// which is used in Stripe JS Elements to handle the card payment
router.post("/create_intent", async (request, response) => {
  try {

    let {items, payment_method_id} = request.body;
    const amount = await calculatePaymentAmount(items);
  
    // Decide if you want to enable installments based on amount.
...
router.post('/confirm_payment', async (request, response) => {
  try {
    ...
    return response.send({success: true, paymentIntent: {status: intent.status}});
  } catch (err) {
    return response.status(500).send({error: err.message});
  }
});
