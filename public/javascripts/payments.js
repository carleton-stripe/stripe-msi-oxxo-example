...
(async () => {
  ...
  const installResetButton = document.querySelector('#reset-installments').addEventListener('click', function() {
    card.clear();
    form.querySelector("#installments-unavailable").hidden = true;
    form.querySelector("#next").textContent = 'Proceder';
    form.querySelector("#next").classList.toggle('visible');
    form.querySelector("#pay").classList.toggle('visible');
  });

  // Check if the Payment Request is available (or Apple Pay on the Web).
  const paymentRequestSupport = await paymentRequest.canMakePayment();
  if ( /* paymentRequestSupport*/ false ) { // disable this button for now.
    // Display the Pay button by mounting the Element in the DOM.
    paymentRequestButton.mount('#payment-request-button');
    // Replace the instruction.
    document.querySelector('.instruction').innerHtml =
      'O completa o <span id="generate">genera</span> tu detalles de envío y pago';
  ...
  const handleInstallmentPlans = async (response) => {
    if (response.error) {
      ...
      const installmentOptions = document.querySelector('#plan-select');
      ...
      if (availablePlans.length > 0) {
        availablePlans.forEach((plan, idx) => {
          ...
          newInput.appendChild(
            document.createTextNode(`${plan.count} meses sin intereses`),
          );
    
          installmentOptions.appendChild(newInput);
          installmentOptions.hidden = false;

        });  
        document.getElementById('immediate-plan').setAttribute('selected', 'selected');
        document.querySelector('.plan').classList.toggle('visible');
      } else {
        form.querySelector("#installments-unavailable").hidden = false;
      }     
  ...
  form.querySelector("#next").addEventListener('click', async event => {
    event.preventDefault();
    await handleReadyForInstallments();
  });
  
  const handleReadyForInstallments = async () => {
    form.querySelector("#next").textContent = 'Cargando...';
    ...
    console.log('paymentMethod is: ' + JSON.stringify(paymentMethod));

    if (error) {
      //alert(error);
      console.log(error);
      form.querySelector("#next").textContent = 'Proceder';
      form.querySelector("#next").disabled = false;
      return;
    }
  ...
  form.addEventListener('submit', async event => {
    ...
    if (payment === 'card') {
      ...
      if (country == 'MX') {
        // Get installment plan and do a server side confirm for Mexico
        const planIdx = form.querySelector('select[name=plan]').value;
        const selectedPlan = availablePlans[planIdx];
        ...
        handlePayment(await response.json());
        return;
      }
      ...
      switch (payment) {
        case 'ideal':
          ...
          let {source} = await stripe.createSource(idealBank, sourceData);
        ...
        case 'ach_credit_transfer':
          ...
          break;
        case 'oxxo': 
            const resp = await stripe.createSource({
              type: 'oxxo',
              amount: paymentIntent.amount,
              currency: paymentIntent.currency,
              owner: {
          ...
          handleSourceActiviation(resp.source);
          return;
      }
      ...
      case 'receiver':
        ...
        switch (source.type) {
          ...
          case 'oxxo':
            console.log(source);
            receiverInfo.innerHTML = `
              <img width="150px" src="https://cdn.glitch.com/4b07d8fe-0ac6-478d-9303-5ed5d5c1220d%2Foxxo.png?v=1571866932777"></img>
            <svg id="barcode"></svg>
            `;
  ...
  const updateButtonLabel = (
    ...
    let label = `Pagar ${amount}`;
    if (paymentMethod !== 'card') {
      form.querySelector("#installments-unavailable").hidden = true;
      form.querySelector("#next").textContent = 'Proceder';
      form.querySelector("#next").classList.remove('visible');
      form.querySelector("#pay").classList.add('visible');
      label = `Pagar ${amount} con ${name}`;
    } else {
      form.querySelector("#installments-unavailable").hidden = true;
      form.querySelector("#next").textContent = 'Proceder';
      form.querySelector("#next").classList.add('visible');
      form.querySelector("#pay").classList.remove('visible');
    }
  ...
  const showRelevantFormFields = country => {
    ...
    form.querySelector('label.zip span').innerText =
      country === 'US' ? 'ZIP' : country === 'GB' ? 'Postcode' : 'Código Postal';
  };
  ...
  for (let input of document.querySelectorAll('input[name=payment]')) {
    input.addEventListener('change', event => {
      ...
      form
        .querySelector('.payment-info.oxxo')
        .classList.toggle('visible', payment === 'oxxo');
      document
